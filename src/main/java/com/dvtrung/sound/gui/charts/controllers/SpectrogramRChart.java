...
import static com.dvtrung.sound.lib.Utils.ZERO_CROSSING_VOICED_SOUND_DIFFERENCE_RANGE;
import static com.dvtrung.sound.lib.Utils.zeroCrossingRate;

public class SpectrogramRChart extends SoundChart {
    ...
    public void plot(double[] waveform) {
        ...
        chart.setParameters(specLog.length * FRAME_COUNT, fftSize2, nyquist);

        for (double[][] s: specLogs) if (s != null) Arrays.stream(s).forEach(chart::addSpecLog);
    }
